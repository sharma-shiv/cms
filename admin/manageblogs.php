...
?>			<script>
	...
	<div id="divcontent">
		...
		<div style="top:50px; left: 30%; width:800px; height:450px; position:relative;">
			<?php if(isset($_SESSION['activate'])){echo $_SESSION['activate']; $_SESSION['activate']="";}?>
			...
			<?php 
				if($result_blog->num_rows>0)
				{
					...
					<tr><th>Cover Image</th><th>User Id</th><th>Title</th><th>Content</th><th colspan=3 style='width:15%;'>Action</tr></tr>";
					while($row = $result_blog->fetch_assoc())
					{
						if($row['status']==2)
						{
							...
							echo "<td class='td' style='width:20%;' >" . substr($row['title'],0,20) . "..</td>";
							echo "<td class='td' style='width:45%;' >" . substr($row['content'],0,50) . "....</td>";
							echo "<td class='td'><a href='viewblog.php?blog_id=".$row['id']."'><img src='../assets/images/view.jpg' height=15px width=15px/></a></td>
							<td class='td'><a href='editblogs.php?blog_id=".$row['id']."'><img src='../assets/images/edit.jpg' height=15px width=15px /></a></td>
							<td class='td'><a href='operator.php?action=deleteblog&data=".$row['id']."' onclick='return confirmation();'><img src='../assets/images/delete.jpg' height=15px width=15px /></a></td>";
							echo "</tr>";
						}
						else
						{
